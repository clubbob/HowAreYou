...
const FAQ_ITEMS = [
  { q: '누가 결제하나요?', a: '보호자만 결제합니다. 보호대상자는 무료로 이용합니다.' },
  {
    ...
    a: '아니요. 보호자 1인당 1회 결제로 보호대상자 인원 제한이 없습니다. 보호대상자가 먼저 보호자를 지정했든, 보호자가 먼저 보호대상자를 지정했든, 이미 구독 중인 보호자는 추가 결제 없이 모두 이용할 수 있습니다.',
  },
  {
  ...
  },
  {
    ...
    a: '홈페이지에서 "1개월 무료로 안심 시작하기" 또는 "베타 1기 신청하기"를 클릭하면 안내 모달이 열립니다. 테스터로 등록된 분은 "Android 앱 설치하기"로 Play Store에서 바로 설치할 수 있고, 참여를 원하시는 분은 "베타 1기 신청하기"에서 이름·휴대폰·이메일을 입력해 신청하시면 됩니다. 신청 후 테스터로 등록되면 설치 링크를 안내해 드립니다.',
  },
  {
    q: '무료 체험은 어떻게 되나요?',
  ...
  },
  {
    ...
    a: `탈퇴하면 데이터는 삭제됩니다. 연 결제(${formatPrice(PRICING.yearly)})는 스토어에서 자동 갱신되므로, 탈퇴만 하면 과금이 계속됩니다. 과금 멈추려면 앱 설정에서 스토어로 이동해 직접 취소하세요. 이미 낸 돈은 환불되지 않습니다.`,
  },
];
