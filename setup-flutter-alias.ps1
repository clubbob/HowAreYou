# Flutter 별칭 설정 스크립트
# 사용: .\setup-flutter-alias.ps1
# 이 스크립트를 실행하면 'flutter run' 대신 'fr'로 실행할 수 있습니다 (EGL 로그 자동 필터링)
...
function fd {
    ...
    `$scriptPath = "$projectPath\flutter-devices.ps1"
...
function frs {
    ...
    `$scriptPath = "$projectPath\run-select-device.ps1"
...
$hasAlias = $profileContent -match "# Flutter Run 별칭"
...
if ($hasAlias) {
    ...
    # 프로필 파일에 추가
    Add-Content -Path $profilePath -Value "`n# Flutter 별칭 (HowAreYou 프로젝트)`n$aliasFunction"
    Write-Host "별칭이 성공적으로 추가되었습니다!" -ForegroundColor Green
    Write-Host "프로필 파일: $profilePath" -ForegroundColor Gray
    Write-Host "`n다음 PowerShell 세션부터 사용 가능한 별칭:" -ForegroundColor Yellow
    Write-Host "  fr  - flutter run (EGL 로그 필터링)" -ForegroundColor White
    Write-Host "  fd  - flutter devices (번호 선택)" -ForegroundColor White
    Write-Host "  frs - flutter run select (에뮬레이터 선택)" -ForegroundColor White
    Write-Host "`n현재 세션에서 사용하려면 다음 명령어 실행:" -ForegroundColor Cyan
