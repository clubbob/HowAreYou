...
- `storeFile`은 **android/ 기준** 고정 경로 (`upload-keystore.jks` = android/upload-keystore.jks)
- `storePassword`와 `keyPassword`는 **반드시 동일**하게 설정
...
**오류 발생 시**: `Keystore was tampered with, or password was incorrect` → [Keystore-비밀번호-오류-해결.md](./Keystore-비밀번호-오류-해결.md) 참고.
