rr# 지금 어때? (HowAreYou) - 상세 PRD (최신 버전)
...
| 항목 | 원본 명세 | 결정 |
|------|----------|------|
| **알림 보내기 (보호자→보호대상자)** | §5.3.5, §9.2.2 | 제외 — 미회신 화면에서 보호대상자에게 FCM 유도 알림 전송 |
| **차트 상단 기분 기반 요약** | §5.2.4, §7.1.7 | 제외 — "최근 7일 중 X일은 '괜찮아' 상태였습니다" 등 기분별 요약 문장 (현재는 "최근 N일 중 X일 기록했어요"만 사용) |
